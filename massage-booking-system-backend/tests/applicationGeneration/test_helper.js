...
const wait = async (howMany)=>{
    let limit = 0
    let response = await Appointment.find()
    while(response.length != howMany && response.length < howMany && limit < 20){
    await sleep(50)
    response = await Appointment.find()
    console.log('how many appointments should be',howMany, 'current amount ', response.length )
    limit++;
    }
  }
  /**
...
module.exports = { emptyTheDatabaseOfAppointments, sleep, wait, loopThroughTheAppointments }