...
describe('with any date', () => {
  ...
  it('generateAppoinmentsForDay leaves a 30 min break between correct appointments', async () => {
    ...
    const beforeBreak = await Appointment.find({
      end_date: '2019-07-20T11:45:00.000Z',
    })
    const afterBreak = await Appointment.find({
      start_date: '2019-07-20T12:15:00.000Z',
    })
  ...
  it('increaseTime returns correct time', () => {
    expect(generator.increaseTime(30, monday).toISOString()).toBe(
      '2019-07-15T09:25:00.000Z'
    )
    expect(generator.increaseTime(110, monday).toISOString()).toBe(
      '2019-07-15T11:15:00.000Z'
    )
  })
